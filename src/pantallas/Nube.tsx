...
export function EstadoSincronizacion(){
 ...
 if(!st.me||!st.nube.vinculada||(!st.nube.guardando&&!st.nube.pendiente&&!st.nube.error&&!error))return null;
 return <View style={{paddingHorizontal:12,paddingTop:ins.top+6,paddingBottom:7,backgroundColor:t.surface,borderBottomWidth:1,borderBottomColor:t.line}}>
  <Row>{st.nube.guardando?<ActivityIndicator color={t.accent}/>:null}<View style={{flex:1}}><Hint>{st.nube.guardando?'Guardando en la nube…':st.nube.error||error||'Cambios pendientes en la copia cifrada del teléfono.'}</Hint></View>
