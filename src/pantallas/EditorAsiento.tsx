...
import {Novedad,Vigilador,Acceso,accNormal,hhmm,addDays,toMin,uid,dmy,personasReales} from '../model';
...
export function EditorNovedad({abierto,novedad,fecha,corte,onClose}:{abierto:boolean;novedad:Novedad|null;fecha:string;corte?:string;onClose:()=>void}){
 ...
 useEffect(()=>{if(!abierto)return;const cat=novedad?categoriaAsiento(novedad):'NOVEDAD';
  ...
  setDia(novedad?.fecha||(corte&&toMin(h)<toMin(corte)?addDays(fecha,1):fecha));setError('');setBorrar(false);
 },[abierto,novedad?.id,fecha]);
 ...
 const guardar=()=>{try{
  ...
  if(corte&&!((dia===fecha&&hora>=corte)||(dia===addDays(fecha,1)&&hora<corte)))throw Error('La fecha y la hora deben pertenecer al período del informe.');
  ...
  st.put('novedades',{...novedad,id:novedad?.id||uid(),fecha:dia,hora,guardId:firma,
  ...
  onClose();toast('Registro guardado en el Informe general');
 ...
 return <Sheet visible={abierto} title={novedad?'Editar registro':'Nueva novedad'} onClose={onClose} footer={<>
  {novedad?<Btn icon="trash" variant="danger" disabled={sinTurno} onPress={()=>setBorrar(true)}/>:null}
  <Btn label="Guardar" variant="primary" disabled={sinTurno} style={{flex:1}} onPress={guardar}/></>}>
